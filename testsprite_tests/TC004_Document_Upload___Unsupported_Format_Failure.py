...
async def run_test():
    ...
    try:
        ...
        # -> Click on the Documents link to navigate to the document management page to attempt uploading unsupported file formats.
        frame = context.pages[-1]
        # Click on the Documents link in the top navigation to go to document management
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Unsupported file format detected').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError("Test case failed: Uploading unsupported document formats did not return the appropriate error message as expected.")
