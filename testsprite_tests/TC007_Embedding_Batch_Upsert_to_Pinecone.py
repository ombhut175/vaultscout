...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Documents' to navigate to document management for embedding generation.
        frame = context.pages[-1]
        # Click on Documents to go to document management for embedding generation
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Embedding Upsert Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Embeddings were not upserted in batches to Pinecone as expected. Batch sizes may have caused throttling or failures, or metadata may be incorrect.")
