...
async def run_test():
    ...
    try:
        ...
        # -> Input admin email and password, then click Continue to log in.
        frame = context.pages[-1]
        # Input admin email
        ...
        # -> Click on the Documents tab or quick link to check for ingestion statuses of uploaded documents.
        frame = context.pages[-1]
        # Click on the Documents tab in the top navigation bar
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Ingestion Complete - All Documents Processed Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The admin dashboard ingestion statuses and system statistics verification did not pass as expected. The expected status 'Ingestion Complete - All Documents Processed Successfully' was not found on the page, indicating a failure in displaying correct ingestion statuses or system statistics.")
