...
async def run_test():
    ...
    try:
        ...
        # -> Click on Documents quick link to navigate to document management for indexing.
        frame = context.pages[-1]
        # Click on Documents quick link to go to document management
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Filtered Results for Nonexistent Organization').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Semantic search did not respect metadata filters such as organization and ACLs, resulting in incorrect search results.")
