...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to authenticate as jane.smith@example.com to access semantic search features
        frame = context.pages[-1]
        # Click on Login to go to authentication page
        ...
        # -> Click on the Search tab in the top navigation bar to access semantic search functionality.
        frame = context.pages[-1]
        # Click on Search tab in the top navigation bar to access semantic search functionality
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Semantic Search Success Rate 100%').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test plan failed: Semantic search functionality did not achieve the required 80% task success rate in predefined relevance smoke tests.")
