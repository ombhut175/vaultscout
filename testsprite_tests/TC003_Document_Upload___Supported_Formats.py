...
async def run_test():
    ...
    try:
        ...
        # Click on the Login link to access the login form
        ...
        # -> Click on Documents link to access the document upload form or API.
        frame = context.pages[-1]
        # Click on Documents link to access document upload form
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Upload Successful for XLSX Document').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Document upload in supported formats (PDF, DOCX, TXT, MD) did not succeed as expected. The test plan execution has failed.")
